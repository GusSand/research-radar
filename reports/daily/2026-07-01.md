...
**Window:** ~June 26 – July 1 2026 (preprints); ICLR 2026 & ICML 2026 newly presented peer-reviewed papers · **Sources swept:** OpenReview, arXiv (cs.CL/cs.LG/cs.CR/cs.AI), ICML 2026 virtual, ACL Anthology, ECCV 2026
...
### 8. [**DLM-Scope: Mechanistic Interpretability of Diffusion Language Models via Sparse Autoencoders**](https://arxiv.org/abs/2602.05859)
...
### 9. [**MaskForge: Structure-Aware Adaptive Attacks for Jailbreaking Diffusion Large Language Models**](https://arxiv.org/abs/2606.04027)
- **Authors / venue:** (multiple authors, including Zhengyue Zhao) — arXiv preprint (cs.CR), June 2026
- **Why it ranks here:** Most recent dLLM security paper in-window; exploits dLLM-native mask-infilling to build reusable attack libraries — a new threat model specific to the dLLM architecture.
- **Technical summary:** Frames dLLM red-teaming as optimized search over a growing library of structural attack patterns (schemas). MaskForge attacks a *joint object* — a user-facing instruction paired with a mask-bearing scaffold — leveraging the dLLM's native infill capability. Successful attempts are abstracted into reusable schemas; a UCB bandit selects goal-compatible patterns from the library; a scorer-guided fallback expands the library when existing patterns fail. This fully black-box approach avoids the low-diversity, uniform-template limitations of prior methods and accumulates attack experience across goals.
...
- **dLLM coverage:** Items #7–#10 form a coherent dLLM cluster: iLLaDA establishes the new capability baseline; DLM-Scope (#8) is the mech-interp lens; MaskForge (#9) is the attack; Steering Without Breaking (#10) is mechanistically-grounded controlled generation. No in-window dLLM *defense* paper stronger than May preprints was found.
...
- Items #8 (DLM-Scope, Feb 2026) may already have been covered in weekly digests; flagged for the weekly roundup deduplication pass.
